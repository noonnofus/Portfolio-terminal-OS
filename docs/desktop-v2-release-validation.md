...
The V2 values were captured on `/gui-v2` before the isolated entry switch. That
component graph later replaced `/gui`, and the current canonical route is
`/desktop`. Both preview routes have been removed.
