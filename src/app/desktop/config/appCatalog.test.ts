...
import {
    appCatalog,
    appCatalogKeys,
} from "@/app/desktop/config/appCatalog";
import { appLoaderRegistryKeys } from "@/app/desktop/lib/appLoaderRegistry";
...
import { collectFolderAppIds } from "@/app/desktop/components/directory/directoryTree";
...
describe("Desktop app boundaries", () => {
    ...
    it("canonicalizes supported Desktop URLs", () => {
        expect(parseDesktopUrl(new URLSearchParams())).toEqual({
            app: "about",
            lang: "ko",
        });
        expect(
            parseDesktopUrl(new URLSearchParams("app=project&slug=wchms&lang=en")),
        ).toEqual({
            app: "project",
            slug: "wchms",
            lang: "en",
        ...
        ).toBe("/en/desktop?app=project&slug=wchms");
        expect(serializeDesktopUrl({ app: "about", lang: "ko" })).toBe("/desktop");
        expect(parseDesktopUrl(new URLSearchParams("app=notes"))).toEqual({
            app: "notes",
            lang: "ko",
        });
        expect(parseDesktopUrl(new URLSearchParams(), "en")).toEqual({
            app: "about",
            lang: "en",
        });
        expect(getDesktopLanguageFromPathname("/en/desktop", "ko")).toBe("en");
        expect(getDesktopLanguageFromPathname("/desktop", "en")).toBe("ko");
    });

    it("falls back through the allowlist for invalid values", () => {
        expect(
            parseDesktopUrl(new URLSearchParams("app=project&slug=unknown")),
        ).toEqual({
            app: "projects",
            lang: "ko",
        });
        expect(
            parseDesktopUrl(new URLSearchParams("app=../../payload&lang=fr")),
        ).toEqual({
            app: "about",
            lang: "ko",
        });
