...
import { DirectoryItem } from "@/app/desktop/components/directory/DirectoryItem";
import type {
  DirectoryFolderNode,
  DirectoryNodeId,
} from "@/app/desktop/components/directory/directoryTypes";
import {
  type Point,
} from "@/app/desktop/hooks/useDraggableAppItem";
import { createOpenAppCommand } from "@/app/desktop/types/appTypes";
import { useDesktopNavigation } from "@/app/desktop/hooks/useDesktopNavigation";
import type { WindowId } from "@/app/desktop/types/navigationTypes";
...
type ItemPositions = Partial<Record<DirectoryNodeId, Point>>;
...
export function DirectorySurface({
  ...
  const itemElements = useRef(
    new Map<DirectoryNodeId, HTMLButtonElement>(),
  );
  const [contentWidth, setContentWidth] = useState(0);
  const [selectedNodeId, setSelectedNodeId] =
    useState<DirectoryNodeId | null>(null);
  const [focusedNodeId, setFocusedNodeId] = useState<DirectoryNodeId | null>(
    directory.children[0]?.nodeId ?? null,
  );
  const [positions, setPositions] = useState<ItemPositions>({});
  const [menuNodeId, setMenuNodeId] = useState<DirectoryNodeId | null>(null);
  ...
  const updatePosition = (nodeId: DirectoryNodeId, point: Point) => {
